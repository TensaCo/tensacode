...
class Registry:
    ...
    def __init__(
        self, strategy: LookupStrategy, dict, default_handler: T_Callable = None
    ) -> None:
        super().__init__()
        self.strategy = strategy
        self.registry = dict or {}
        if default_handler is not None:
            self.register(ConstFn(True), default_handler)

    # Registration methods

    def register(self, key_or_fn: Hashable | T_Callable, value: _V):
    ...
    def _all_iter(self, dict, fn_args, fn_kwargs):
        for fn, val in dict.items():
            try:
                ...
                logging.warning(
                    f"Trigger function {fn} does not coform to `Predicate` protocol. Skipping."
                )
    ...
    def _all_iter(self, dict, fn_args, fn_kwargs):
        ...
        for fn, val in dict.items():
            try:
                ...
                logging.warning(
                    f"Scoring function {fn} does not coform to `Predicate` protocol. Skipping."
                )
